...
# WORKED EXAMPLE 1 — Demographics Table (ADSL)
#
# Shell has: Age (Years) with n, Mean (SD), Median, Min, Max;
#            Sex with Male/Female rows showing n (%);
#            Race with multiple category rows showing n (%)
# Columns: Placebo (N=xx), Drug A (N=xx), Total (N=xx)
...
#       "add_total": true,
#       "layers": [
#         {"type": "group_desc", "var": "AGE", "nested_var": null, "by_var": null,
#          "distinct_by": null, "stats": ["n", "mean", "sd", "median", "min", "max"]},
#         {"type": "group_count", "var": "SEX", "nested_var": null, "by_var": null,
#          "distinct_by": null},
#         {"type": "group_count", "var": "RACE", "nested_var": null, "by_var": null,
#          "distinct_by": null}
#       ]
#     }
#   ],
...
#
# KEY POINTS:
# - ONE layer for AGE (group_desc), ONE for SEX, ONE for RACE — not one per category
# - by_var is null for all layers — Tplyr auto-creates rows for each unique value
# - pre_filters captures the population flag ITTFL
# - treatment_var uses the actual column name TRT01P from the dataset

# WORKED EXAMPLE 2 — AE Table by SOC and Preferred Term (ADAE)
#
# Shell has: Subjects with any TEAE; then SOC rows with indented PT rows
# Columns: Placebo (N=xx), Drug A 50mg (N=xx), Total (N=xx)
# Population: Safety Analysis Set (SAFFL=Y); Treatment-Emergent (TRTEMFL=Y)
#
# CORRECT recipe:
# {
#   "approach": "tplyr",
...
#       "add_total": true,
#       "layers": [
#         {"type": "group_count", "var": "any_event", "nested_var": null,
#          "by_var": null, "distinct_by": "USUBJID"},
...
#
# KEY POINTS:
# - "Subjects with any TEAE" uses a derived var "any_event" with distinct_by USUBJID
# - SOC/PT is ONE nested layer: var=AEBODSYS, nested_var=AEDECOD — NOT two layers
# - SAFFL and TRTEMFL are pre_filters, NEVER used as var/nested_var
# - distinct_by=USUBJID on both layers for subject-level counting

# WORKED EXAMPLE 3 — Best Overall Response Table (ADRS)
#
# Shell has: BOR categories (CR, PR, SD, PD, NE) with n (%);
#            ORR (CR+PR) derived row; DCR (CR+PR+SD) derived row
# Columns: Drug A (N=xx), Placebo (N=xx)
# Population: Full Analysis Set (FASFL=Y), PARAMCD=BOR, ANL01FL=Y
#
# CORRECT recipe:
# {
#   "approach": "tplyr",
#   "dataset_var": "adrs",
#   "pre_filters": ["FASFL == 'Y'", "PARAMCD == 'BOR'", "ANL01FL == 'Y'"],
#   "derived_vars": [
#     {"dataset_var": "adrs", "name": "ORR_FLAG", "expr": "ifelse(AVALC %in% c('CR','PR'), 'Yes', 'No')"},
#     {"dataset_var": "adrs", "name": "DCR_FLAG", "expr": "ifelse(AVALC %in% c('CR','PR','SD'), 'Yes', 'No')"}
#   ],
#   "tables": [
#     {
#       "table_var": "t1",
...
#       "add_total": false,
#       "layers": [
#         {"type": "group_count", "var": "AVALC", "nested_var": null,
#          "by_var": null, "distinct_by": "USUBJID"}
#       ]
#     },
#     {
...
#       "add_total": false,
#       "layers": [
#         {"type": "group_count", "var": "ORR_FLAG", "nested_var": null,
#          "by_var": null, "distinct_by": "USUBJID"},
#         {"type": "group_count", "var": "DCR_FLAG", "nested_var": null,
#          "by_var": null, "distinct_by": "USUBJID"}
#       ]
#     }
#   ],
...
#
# KEY POINTS:
# - ONE layer for all BOR categories — Tplyr auto-creates CR/PR/SD/PD/NE rows
# - ORR and DCR are derived flags, each in their own layer in a second table
# - PARAMCD filter is in pre_filters, not a layer variable
# - AVALC is the actual column name from AdaM specs — never invent names
"""
